...
def load_models():
    ...
    try:
        # 关键修复：Streamlit Cloud 是 CPU 环境，必须把训练时保存在 CUDA 上的模型映射到 CPU。
        model.load_net(
        ...
        baseline = joblib.load(BASE_DIR / "baseline_hazards.joblib")
        # 兼容不同 pycox 版本：有的版本读取 baseline_hazards_，有的读取 baseline_cumulative_hazards_
        model.baseline_hazards_ = baseline
        model.baseline_cumulative_hazards_ = baseline
        income_quantiles = joblib.load(BASE_DIR / "income_quantiles.joblib")
    except Exception as e:
        raise RuntimeError(
            "模型文件加载失败。常见原因包括：\n"
            "1) 训练时在 GPU 保存，部署时在 CPU 加载；\n"
            "2) scikit-learn / torch / pycox / torchtuples 版本不一致。\n"
            f"原始错误: {e}"
        ) from e
